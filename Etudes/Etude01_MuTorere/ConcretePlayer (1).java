...
class ConcretePlayer extends Player {
	...
	/**
	we modified getmove to examine possible losing positions when choosing a move to make
	we hard code all losing positions and rotate and reflect them, comparing the current game state to these
	we choose any other move that is not known to result in a losing position
	we do this for both players so that we take different moves for player one/two
	 */
	public int getMove() {
